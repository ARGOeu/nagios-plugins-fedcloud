...
# this should be already activated in the system libs, but just redo in case
# it's missing. This should support SNI
try:
...
def get_keystone_oidc_unscoped_token(parsed_url, suffix, timeout, capath, token,
...
def get_keystone_x509_unscoped_token(parsed_url, suffix, timeout, capath, userca,
    ...
    try:
        ...
        response = requests.post(parsed_url.scheme+'://'+ parsed_url.netloc + token_suffix, headers=headers,
                                 cert=userca, verify=capath, timeout=timeout)
...
def get_keystone_v3_token(unscoped_token_getter, host, capath, timeout, **kwargs):
    ...
    token = unscoped_token_getter(o, suffix, timeout, capath, **kwargs)
    ...
    try:
        ...
        response = requests.get(o.scheme+'://'+o.netloc+project_suffix, headers=headers,
                                data=None, timeout=timeout, verify=capath)
        response.raise_for_status()
    ...
    try:
        ...
        response = requests.post(o.scheme+'://'+o.netloc+token_suffix, headers=headers,
                                data=json.dumps(payload), verify=capath, timeout=timeout)
        response.raise_for_status()
...
def get_keystone_token_x509_v2(host, capath, timeout, userca=None):
    ...
    try:
        ...
        response = requests.post(o.scheme+'://'+o.netloc+token_suffix, headers=headers,
                                data=json.dumps(payload), cert=userca, verify=capath, timeout=timeout)
        response.raise_for_status()
    ...
    try:
        ...
        response = requests.get(o.scheme+'://'+o.netloc+tenant_suffix, headers=headers,
                                data=None, cert=userca, verify=capath, timeout=timeout)
        response.raise_for_status()
